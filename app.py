...
# SVG STRINGS (Optimized for Streamlit)
ICON_DESKTOP = """<svg viewBox="0 0 1024 1024" class="icon" style="width: 1.8em; height: 1.8em; vertical-align: middle; margin-right: 15px;" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M511.3 676.9m-10 0a10 10 0 1 0 20 0 10 10 0 1 0-20 0Z" fill="#E73B37"></path><path d="M960 756V138.5H64V756h320.1v85.5H256.2v44h511.9v-44h-128V756H960zM108 182.5h808v427.1H108V182.5z m488.1 659h-168V756h168v85.5zM108 712v-82.5h808V712H108z" fill="#ffffff"></path><path d="M167.536 327.703l90.72-90.721 14.143 14.142-90.721 90.72zM172.959 423.469l181.159-181.16 14.142 14.143L187.1 437.61z" fill="#E73B37"></path></g></svg>"""

ICON_FLASK = """<svg viewBox="0 0 1024 1024" class="icon" style="width: 1.5em; height: 1.5em; vertical-align: middle; margin-right: 10px;" version="1.1" xmlns="http://www.w3.org/2000/svg" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M517.8 811c17.1 0 31 13.9 31 30.9 0 17.1-13.9 30.9-31 30.9s-31-13.9-31-30.9 13.9-30.9 31-30.9m0-20c-28.1 0-51 22.8-51 50.9 0 28.1 22.8 50.9 51 50.9s51-22.8 51-50.9c-0.1-28.1-22.9-50.9-51-50.9zM334.218 304.477l65.69-65.69 14.142 14.143-65.69 65.69zM352.086 387.799L483.253 256.63l14.142 14.142-131.167 131.168z" fill="#E73B37"></path><path d="M808 108.4v807.2H216V108.4h592m44-44H172v895.2h680V64.4z" fill="#ffffff"></path><path d="M747.1 173.4v572.9H279.9V173.4h467.2m22-22H257.9v616.9h511.2V151.4z" fill="#ffffff"></path></g></svg>"""
...
def render_header(svg_code, text, level=2):
    """Helper to render SVG icons next to header text."""
    html = f"""
...
st.sidebar.markdown(f"""<div style="display: flex; align-items: center;">{ICON_CALENDAR}<h2 style="display: inline; font-size: 1.5rem;">Experiment Planning</h2></div>""", unsafe_allow_html=True)
...
with st.sidebar.expander("Step 1: Sample Size Calculator", expanded=False):
    ...
    # Inputs
    plan_traffic_28d = st.number_input("Traffic (Last 28 Days)", value=50000, step=1000, help="Total users in the last 4 weeks.")
    ...
    plan_mde = st.number_input("Min Detectable Effect (%)", value=5.0, step=0.5, help="The smallest lift you want to detect.")
    ...
    if st.button("Calculate Duration"):
        ...
        st.write(f"**Target Lift:** {plan_mde}%")
        ...
        if days_rpv > 60:
            ...
            st.caption("Warning: Proving Revenue lift is extremely hard with high variance.")
        else:
...
def get_ai_analysis(api_key, hypothesis, metrics_dict, provider="OpenAI", conf_level="95%"):
    ...
    try:
        ...
        **Configuration:**
        - Confidence Level Used: {conf_level}
        ...
        **Metrics:**
        ...
        - {conf_level} Confidence Interval (Absolute Diff): {metrics_dict['ci_low']:.2f}% to {metrics_dict['ci_high']:.2f}%
        ...
        **Task:**
        1. Executive Summary (Win/Loss/Inconclusive based on p < {metrics_dict['alpha']}).
        ...
        **5. Visual Analysis (Graph-by-Graph):**
        ...
        - Strategic Matrix (CR vs AOV).
        - Product Metrics (Items per order).
        - Revenue Charts (RPV/AOV).
        - CR Comparison.
        - Bayesian (Probability density).
        - Bootstrap (Histogram and CI).
...
def generate_smart_analysis(hypothesis, metrics, alpha_val):
    ...
    if metrics['days'] < 7:
        report.append(f"Duration Warning: Test ran for {metrics['days']} days (too short).")
    elif metrics['days'] < 14:
        report.append(f"Duration Caution: Test ran for {metrics['days']} days (watch for novelty effects).")
    else:
    ...
    if metrics['prob_v_wins'] > 95:
        ...
        report.append("Uncertain: The probability is between 5% and 95%, meaning the winner is not yet clear.")
    ...
    if metrics['loss_v'] < 0.01:
        ...
        report.append("Material Risk: Switching now carries a potential cost in conversion rate.")
    ...
    report.append("\n**2. Bootstrap & CI:**")
    if metrics['ci_low'] > 0:
        report.append(f"The {confidence_level} Confidence Interval ({metrics['ci_low']:.2f}% to {metrics['ci_high']:.2f}%) is entirely positive.")
    elif metrics['ci_high'] < 0:
        report.append(f"The {confidence_level} Confidence Interval is entirely negative.")
    else:
        report.append(f"The {confidence_level} Confidence Interval ({metrics['ci_low']:.2f}% to {metrics['ci_high']:.2f}%) crosses zero, indicating uncertainty.")
...
with tab1:
    ...
    if st.button("Generate Smart Analysis"):
        # 1. Calc CI using selected alpha
