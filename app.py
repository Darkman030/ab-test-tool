...
matplotlib.use('Agg') # Prevents threading crashes

# --- 2. ASSETS & ICONS (DEFINED FIRST) ---
...
with st.sidebar.expander("Sample Size Calculator", expanded=False):
    ...
    if st.button("Calculate Duration"):
        # ZeroDivisionError Fix: Check if inputs are valid
        if plan_base_aov <= 0 or plan_base_cr <= 0:
            st.error("Baseline AOV and CR must be greater than 0.")
        else:
            ...
            # Additional safety check for zero standard deviation
            if est_sd == 0:
                effect_size_rpv = 0
                n_rpv = 0
            else:
            ...
            n_rpv_visitors = n_rpv / (plan_base_cr / 100) if plan_base_cr > 0 else 0
            
            days_cr = (n_cr * 2) / daily_traffic if daily_traffic > 0 else 0
            days_rpv = (n_rpv_visitors * 2) / daily_traffic if daily_traffic > 0 else 0
