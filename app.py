...
def generate_comprehensive_report(hypothesis, metrics, alpha_val):
    ...
    if metrics['loss_v'] < 0.01:
        report.append("- Risk: LOW. Expected loss from switching is negligible.")
    else:
...
def get_ai_analysis(api_key, hypothesis, metrics_dict, provider="OpenAI", conf_level="95%"):
    if not api_key: return "Please enter a valid API Key."
    ...
    try:
        ...
        prompt = f"Analyze this A/B test results. Config: {conf_level}. Hypothesis: {hypothesis}. Data: {metrics_dict}. Provide executive summary, trade-offs, and risk assessment."
...
with st.sidebar.expander("Open Controls", expanded=False):
    st.info("Workflow Continuity: Capture a snapshot. Download on Day 7, re-upload on Day 14.")
...
# --- 5. MAIN CALCULATIONS & UI ---
# Calculations using Session State
