...
# FIX: Set Matplotlib backend to Agg to prevent threading crashes in Streamlit
matplotlib.use('Agg')
...
# MAIN PAGE HEADER
# ==============================================
st.title("A/B Test Analyzer v1.8")
...
with st.sidebar.expander("Open Controls", expanded=False):
    ...
    current_state = {
        ...
        "s1_uc": st.session_state.get("s1_uc", 0), "s1_cc": st.session_state.get("s1_cc", 0),
        "s1_uv": st.session_state.get("s1_uv", 0), "s1_cv": st.session_state.get("s1_cv", 0),
        "s2_uc": st.session_state.get("s2_uc", 0), "s2_cc": st.session_state.get("s2_cc", 0),
        "s2_uv": st.session_state.get("s2_uv", 0), "s2_cv": st.session_state.get("s2_cv", 0)
    }
    st.download_button(
...
# ==============================================
st.sidebar.markdown(f"""<div style="display: flex; align-items: center;">{ICON_TROPHY}<h2 style="display: inline; font-size: 1.5rem; margin-left: 5px;">Enter Results</h2></div>""", unsafe_allow_html=True)
...
# --- NEW: RULE-BASED NLP ENGINE ---
def generate_smart_analysis_v2(hypothesis, metrics, alpha_val):
    report = []
    ...
    if sig and uplift > 0:
        ...
        summary = f"The variation has proven to be statistically superior with {100-metrics['alpha']*100:.0f}% confidence."
    elif sig and uplift < 0:
        headline = "❌ LOSER: Significant Negative Result"
        summary = f"The variation is performing significantly worse than the control. Immediate rollback recommended."
    else:
        headline = "⚠️ INCONCLUSIVE: No Clear Winner Yet"
        summary = f"We cannot reject the null hypothesis (p={metrics['p_cr']:.3f}). The difference observed could be due to random chance."

    report.append(f"### {headline}")
    report.append(summary)
    
    # 2. Key Drivers (The "Why")
    report.append("#### 🔍 Key Drivers")
    drivers = []
    
    # CR Logic
    if abs(metrics['uplift_cr']) < 1.0: drivers.append("Conversion Rate remained stable (flat).")
    elif metrics['uplift_cr'] > 0: drivers.append(f"More users are converting (**+{metrics['uplift_cr']:.1f}%** CR).")
    else: drivers.append(f"Fewer users are converting (**{metrics['uplift_cr']:.1f}%** CR).")
    
    # AOV Logic
    if abs(metrics['uplift_aov']) < 1.0: drivers.append("Spending power (AOV) remained stable.")
    elif metrics['uplift_aov'] > 0: drivers.append(f"Users are spending more per order (**+${metrics['aov_v']-metrics['aov_c']:.2f}**).")
    else: drivers.append(f"Users are spending less per order (**-${metrics['aov_c']-metrics['aov_v']:.2f}**).")
    
    report.append("- " + "\n- ".join(drivers))
    
    # 3. Strategic Interpretation (The Matrix)
    report.append("#### 🧠 Strategic Interpretation")
    if metrics['uplift_cr'] > 0 and metrics['uplift_aov'] > 0:
        report.append("🚀 **Growth Engine:** You have achieved the 'Holy Grail'—more orders AND higher value. This is a highly efficient win.")
    elif metrics['uplift_cr'] > 0 and metrics['uplift_aov'] < 0:
        report.append("📉 **Discount Effect:** You are driving more volume, but value is diluting. Ensure the extra orders cover the margin loss.")
    elif metrics['uplift_cr'] < 0 and metrics['uplift_aov'] > 0:
        report.append("💎 **Premium Shift:** You are filtering out low-quality leads. Fewer orders, but they are worth more. Good for profitability.")
    else:
        report.append("🛑 **Negative Friction:** You are hurting both conversion and value. Something in the UX is likely broken.")

    # 4. Recommendation
    report.append("#### 💡 Recommendation")
    if sig and uplift > 0:
        report.append("🚀 **Ship It:** The results are robust. Roll out to 100% traffic.")
    elif sig and uplift < 0:
        report.append("🔙 **Roll Back:** The variation is hurting performance. Revert to Control.")
    elif metrics['days'] < 14:
        report.append("⏳ **Wait:** The test has run for less than 2 full business cycles. Collect more data.")
    else:
        report.append("🛑 **Stop & Iterate:** Data is flat after 14+ days. The change was likely too subtle to matter. Try a bolder test.")
...
# --- TAB 1: SMART ANALYSIS (NEW AUTO-INSIGHTS) ---
with tab1:
    ...
    if st.button("Generate Report"):
        ...
        # CALL THE NEW V2 GENERATOR
        smart_result = generate_smart_analysis_v2("N/A", metrics_payload, alpha)
...
with tab3:
    ...
    st.caption("If you peek at results multiple times, you increase the False Positive rate. This calculator adjusts your Alpha.")
    peeks = st.number_input("How many times have you checked the results?", min_value=1, value=1)
    
    # Simple pocock-like penalty approximation or Alpha Spend
    adjusted_alpha = alpha * np.log(1 + (np.e - 1) / peeks)
    
    st.write(f"Standard Significance Threshold: **{alpha}**")
    st.write(f"Adjusted Threshold (Penalty): **{adjusted_alpha:.5f}**")
    
    if p_value_z < adjusted_alpha: 
        st.success(f"SIGNIFICANT (p={p_value_z:.4f}) even after penalty.")
    else: 
