...
# --- A. SAVE / LOAD STATE ---
with st.sidebar.expander("💾 Save & Load Analysis", expanded=True):
    st.info("Download your current inputs as a file to resume later without re-typing.")
    
    # 1. Download State
    # Collect all current values
    current_state = {
        "users_c": st.session_state.get("users_c", 5000),
        "conv_c": st.session_state.get("conv_c", 500),
        "rev_c": st.session_state.get("rev_c", 25000.0),
        "prod_c": st.session_state.get("prod_c", 750),
        "users_v": st.session_state.get("users_v", 5000),
        "conv_v": st.session_state.get("conv_v", 600),
        "rev_v": st.session_state.get("rev_v", 33000.0),
        "prod_v": st.session_state.get("prod_v", 1000),
        "days": st.session_state.get("days", 14),
...
with st.sidebar.expander("Step 1: Sample Size Calculator", expanded=False):
    ...
    plan_traffic_28d = st.number_input("Traffic (Last 28 Days)", value=50000, step=1000, key="p_traffic")
    plan_base_cr = st.number_input("Baseline Conversion Rate (%)", value=2.5, step=0.1, key="p_cr")
    plan_base_aov = st.number_input("Baseline AOV ($)", value=75.0, step=1.0, key="p_aov")
    plan_mde = st.number_input("Min Detectable Effect (%)", value=5.0, step=0.5, key="p_mde")
...
users_control = st.sidebar.number_input("Control Users", min_value=1, value=5000, key="users_c")
conv_control = st.sidebar.number_input("Control Conversions", min_value=0, value=500, key="conv_c")
rev_control = st.sidebar.number_input("Control Revenue ($)", min_value=0.0, value=25000.0, key="rev_c")
prod_control = st.sidebar.number_input("Control Products Sold", min_value=0, value=750, key="prod_c")

st.sidebar.markdown("---")

# Variation
st.sidebar.subheader("Variation Group")
users_variation = st.sidebar.number_input("Variation Users", min_value=1, value=5000, key="users_v")
conv_variation = st.sidebar.number_input("Variation Conversions", min_value=0, value=600, key="conv_v")
rev_variation = st.sidebar.number_input("Variation Revenue ($)", min_value=0.0, value=33000.0, key="rev_v")
prod_variation = st.sidebar.number_input("Variation Products Sold", min_value=0, value=1000, key="prod_v")

st.sidebar.markdown("---")
days_run = st.sidebar.number_input("Days Test Ran", min_value=1, value=14, key="days")
...
with tab1:
    ...
    if st.button("Generate Smart Analysis"):
        ...
        st.markdown("---")
        # Collapsed Copy Section
        with st.expander("📋 View & Copy Full Report", expanded=False):
