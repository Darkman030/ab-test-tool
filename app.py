...
# MAIN PAGE HEADER
# ==============================================
st.title("A/B Test Analyzer v1.9")
...
# --- NEW: COMPREHENSIVE AUTO-ANALYST (No Emojis) ---
def generate_comprehensive_report(hypothesis, metrics, alpha_val):
    ...
    if sig and uplift > 0:
        ...
        summary = f"The variation is performing significantly worse than the control."
    else:
    ...
    # 3. Deep Dive: Secondary Metrics (Product Velocity)
    ...
    if metrics['p_cr'] < metrics['adjusted_alpha']:
        report.append(f"- Sequential Testing: ROBUST. The result remains significant even after accounting for peeking penalties (Adjusted Alpha: {metrics['adjusted_alpha']:.5f}).")
    else:
        report.append(f"- Sequential Testing: CAUTION. While the standard test might show significance, frequent peeking increases the false positive risk. The adjusted threshold was not met.")

    # Bayesian
    report.append(f"- Bayesian Probability: There is a {metrics['prob_v_wins']:.1f}% chance the variation is the true winner.")
    if metrics['loss_v'] < 0.01:
        report.append("- Risk: LOW. The expected loss from switching is negligible.")
    else:
        report.append(f"- Risk: MODERATE/HIGH. Switching carries a potential loss of {metrics['loss_v']:.4f}.")

    # Bootstrap / Box Plot Proxy
    width = metrics['ci_high'] - metrics['ci_low']
    if width < 5.0:
        report.append(f"- Bootstrap Confidence: PRECISE. The confidence interval is narrow ({width:.2f}%), indicating stable behavior across user samples.")
    else:
        report.append(f"- Bootstrap Confidence: VOLATILE. The confidence interval is wide ({width:.2f}%), suggesting high variance in user behavior.")

    # 5. Strategic Conclusion
    report.append("#### Strategic Conclusion")
    if metrics['uplift_cr'] > 0 and metrics['uplift_aov'] > 0:
        report.append("Growth Engine: You have achieved a highly efficient win, driving both volume and value.")
    elif metrics['uplift_cr'] > 0 and metrics['uplift_aov'] < 0:
        report.append("Discount Effect: You are driving volume at the expense of value. Monitor margins.")
    elif metrics['uplift_cr'] < 0 and metrics['uplift_aov'] > 0:
        report.append("Premium Shift: You are filtering for higher-value customers, but losing volume.")
    else:
        report.append("Negative Friction: Both conversion and value are down. Revert immediately.")
...
with tab1:
    ...
    # Calculate Sequential Penalty for the report
    peeks_for_report = 1 # Default assumption
