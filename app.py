...
# --- 4. HELPER FUNCTIONS (PRE-DEFINED TO AVOID NAMERROR) ---
...
def calculate_uplift(ctrl, var):
    return ((var - ctrl) / ctrl) * 100 if ctrl > 0 else 0.0
...
def generate_comprehensive_report(hypothesis, metrics, alpha_val):
    ...
    if metrics['p_cr'] < metrics['adjusted_alpha']:
        ...
        report.append(f"- Sequential Testing: CAUTION. While the standard test might show significance, frequent peeking increases the false positive risk.")
    ...
    if width < 5.0:
        report.append(f"- Bootstrap Confidence: PRECISE. The confidence interval is narrow ({width:.2f}%), indicating stable behavior.")
    else:
        report.append(f"- Bootstrap Confidence: VOLATILE. The confidence interval is wide ({width:.2f}%), indicating high variance.")
...
# --- 5. MAIN UI LAYOUT ---
# Sidebar
st.sidebar.markdown(f"""<div style="display: flex; align-items: center; margin-bottom: 10px;">{ICON_SETTINGS}<h2 style="margin: 0; padding: 0;">Settings</h2></div>""", unsafe_allow_html=True)

# Save/Load
st.sidebar.markdown(f"""<div style="display: flex; align-items: center; margin-bottom: 10px;">{ICON_UPLOAD}<h2 style="margin: 0; padding: 0;">Save & Load Analysis</h2></div>""", unsafe_allow_html=True)
with st.sidebar.expander("Open Controls", expanded=False):
    st.info("Workflow Continuity: Capture a snapshot of your experiment. Download data on Day 7, then re-upload on Day 14.")
    ...
    st.download_button(label="Download Snapshot (.json)", data=json.dumps(current_state, indent=2), file_name="experiment_data.json", mime="application/json")
...
# Planning
st.sidebar.markdown(f"""<div style="display: flex; align-items: center; margin-bottom: 10px;">{ICON_CALENDAR}<h2 style="margin: 0; padding: 0;">Experiment Planning</h2></div>""", unsafe_allow_html=True)
...
# Enter Results
st.sidebar.markdown(f"""<div style="display: flex; align-items: center; margin-bottom: 10px;">{ICON_TROPHY}<h2 style="margin: 0; padding: 0;">Enter Results</h2></div>""", unsafe_allow_html=True)
...
with st.sidebar.expander("Input Data", expanded=True):
    ...
    users_control = st.number_input("Users", min_value=1, key="users_c")
    conv_control = st.number_input("Conversions", min_value=0, key="conv_c")
    rev_control = st.number_input("Revenue ($)", min_value=0.0, key="rev_c")
    prod_control = st.number_input("Items", min_value=0, key="prod_c")
    st.markdown("---")
    st.subheader("Variation Group")
    users_variation = st.number_input("Users", min_value=1, key="users_v")
    conv_variation = st.number_input("Conversions", min_value=0, key="conv_v")
    rev_variation = st.number_input("Revenue ($)", min_value=0.0, key="rev_v")
    prod_variation = st.number_input("Items", min_value=0, key="prod_v")
    st.markdown("---")
    days_run = st.number_input("Days Test Ran", min_value=1, key="days")

# --- 6. CORE MATH & MAIN DASHBOARD ---
