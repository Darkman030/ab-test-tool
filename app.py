...
# ==============================================
# 0. STATE INITIALIZATION (Fixes Warnings)
# ==============================================
...
alpha_val = {"90%": 0.10, "95%": 0.05, "99%": 0.01}[confidence_level]
st.sidebar.markdown("---")

# Save & Load
save_load_help = "Weekly Workflow: Capture a snapshot of your test. Download on Day 7, re-upload on Day 14 to see evolution."
...
with st.sidebar.expander("Open Controls", expanded=False):
    current_state = {k: st.session_state[k] for k in ["users_c", "conv_c", "rev_c", "prod_c", "users_v", "conv_v", "rev_v", "prod_v", "days", "conf_level", "p_traffic", "p_base_cr", "p_base_aov", "p_mde", "p_vol"]}
...
with st.sidebar.expander("Calculator", expanded=False):
    st.number_input("Traffic (Last 28 Days)", step=1000, key="p_traffic")
    st.number_input("Baseline CR (%)", step=0.1, key="p_base_cr")
    st.number_input("Baseline AOV ($)", step=1.0, key="p_base_aov")
    st.number_input("Target Lift (%)", step=0.5, key="p_mde")
    st.selectbox("Revenue Variance", ["Low (Subscription)", "Medium (Standard E-com)", "High (Whales/B2B)"], key="p_vol")
    
    if st.button("Calculate Duration"):
        daily = st.session_state["p_traffic"] / 28
        p1 = st.session_state["p_base_cr"] / 100
        e_size = proportion_effectsize(p1, p1 * (1 + st.session_state["p_mde"]/100))
        n = NormalIndPower().solve_power(effect_size=e_size, alpha=alpha_val, power=0.8, ratio=1)
        st.info(f"Need {int((n*2)/daily)} days traffic.")
...
st.sidebar.number_input("Users", min_value=1, key="users_c")
st.sidebar.number_input("Conversions", min_value=0, key="conv_c")
st.sidebar.number_input("Revenue ($)", min_value=0.0, key="rev_c")
st.sidebar.number_input("Items", min_value=0, key="prod_c")
st.sidebar.markdown("---")
st.sidebar.subheader("Variation Group")
st.sidebar.number_input("Users", min_value=1, key="users_v")
st.sidebar.number_input("Conversions", min_value=0, key="conv_v")
st.sidebar.number_input("Revenue ($)", min_value=0.0, key="rev_v")
st.sidebar.number_input("Items", min_value=0, key="prod_v")
st.sidebar.markdown("---")
st.sidebar.number_input("Days Test Ran", min_value=1, key="days")
...
def calculate_bayesian_risk(alpha_c, beta_c, alpha_v, beta_v, num_samples=50000):
    ...
    return prob_v_wins, loss_v

rate_c, rate_v = st.session_state["conv_c"]/st.session_state["users_c"], st.session_state["conv_v"]/st.session_state["users_v"]
up_cr = calculate_uplift(rate_c, rate_v)
aov_c, aov_v = (st.session_state["rev_c"]/st.session_state["conv_c"] if st.session_state["conv_c"] > 0 else 0), (st.session_state["rev_v"]/st.session_state["conv_v"] if st.session_state["conv_v"] > 0 else 0)
up_aov = calculate_uplift(aov_c, aov_v)
rpv_c, rpv_v = st.session_state["rev_c"]/st.session_state["users_c"], st.session_state["rev_v"]/st.session_state["users_v"]
z_stat, p_val = proportions_ztest([st.session_state["conv_c"], st.session_state["conv_v"]], [st.session_state["users_c"], st.session_state["users_v"]])
srm_chi, p_srm = chi2_contingency([[st.session_state["users_c"], st.session_state["users_v"]], [(st.session_state["users_c"]+st.session_state["users_v"])/2]*2])[:2]
prob_v, loss_v = calculate_bayesian_risk(st.session_state["conv_c"]+1, st.session_state["users_c"]-st.session_state["conv_c"]+1, st.session_state["conv_v"]+1, st.session_state["users_v"]-st.session_state["conv_v"]+1)
...
c4.success(f"Sig: {p_val <= alpha_val} (p={p_val:.4f})")
...
with st.expander("Expand Segments", expanded=False):
    ...
    with col_s1:
        ...
        st.number_input("S1 Control Users", key="s1_uc"); st.number_input("S1 Control Conv.", key="s1_cc")
        st.number_input("S1 Var Users", key="s1_uv"); st.number_input("S1 Var Conv.", key="s1_cv")
    with col_s2:
        st.subheader("Segment 2")
        st.number_input("S2 Control Users", key="s2_uc"); st.number_input("S2 Control Conv.", key="s2_cc")
        st.number_input("S2 Var Users", key="s2_uv"); st.number_input("S2 Var Conv.", key="s2_cv")
    
    if st.button("Check Paradox"):
        up1 = calculate_uplift(st.session_state["s1_cc"]/st.session_state["s1_uc"] if st.session_state["s1_uc"]>0 else 0, st.session_state["s1_cv"]/st.session_state["s1_uv"] if st.session_state["s1_uv"]>0 else 0)
        up2 = calculate_uplift(st.session_state["s2_cc"]/st.session_state["s2_uc"] if st.session_state["s2_uc"]>0 else 0, st.session_state["s2_cv"]/st.session_state["s2_uv"] if st.session_state["s2_uv"]>0 else 0)
        st.write(f"S1 Uplift: {up1:.2f}% | S2 Uplift: {up2:.2f}%")
...
with tab1:
    ...
    st.markdown(f"**Executive Summary:** Test is {'Significant' if p_val <= alpha_val else 'Inconclusive'}.")
    st.markdown(f"**Data Health:** SRM Check {'Passed' if p_srm > 0.01 else 'FAILED'} (p={p_srm:.4f}).")
    st.markdown(f"**Bayesian Risk:** Prob. to be Best is {prob_v*100:.1f}%. Expected loss if switching: {loss_v*100:.5f}%.")
...
with tab2:
    st.info("AI Analysis module active. Enter hypothesis to proceed.")
...
with tab4:
    ...
    y_c = beta(st.session_state["conv_c"]+1, st.session_state["users_c"]-st.session_state["conv_c"]+1).pdf(x)
    y_v = beta(st.session_state["conv_v"]+1, st.session_state["users_v"]-st.session_state["conv_v"]+1).pdf(x)
    fig, ax = plt.subplots()
    ...
    ax.set_title("Probability Density Curves"); ax.legend(); st.pyplot(fig)

with tab5:
    sim_c = np.random.binomial(int(st.session_state["users_c"]), rate_c, 5000)/st.session_state["users_c"]
    sim_v = np.random.binomial(int(st.session_state["users_v"]), rate_v, 5000)/st.session_state["users_v"]
    diffs = (sim_v - sim_c) * 100
    fig, ax = plt.subplots()
    ax.hist(diffs, bins=50, color='orange', alpha=0.7, edgecolor='black')
    ax.axvline(0, color='red', linestyle='--')
    ax.set_title("Distribution of Lift (%)"); st.pyplot(fig)
